...
const AdminFaqEdit = (props) => {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_SERVER_URL}/admin/faq/${params.id}`)
      .then((res) => {
  ...
  const postData = async () => {
    ...
    await axios
      .put(
        `${process.env.REACT_APP_SERVER_URL}/admin/faq/${params.id}/update`,
        formData
      )
      .then((res) => {
        setFlag(false);
  ...
  return (
    <div>
      <Helmet>
        <title>Acres - Real Estate React Template | Admin Faq Edit</title>
